...
const StallsList = () => {
  ...
  const handleHeartClick = (stall) => {
    toggleHeart(stall);
    toast({
      title: isLiked(stall.id) ? "Removed from interests" : "Added to interests",
      description: isLiked(stall.id)
        ? `${stall.name} removed from your interests`
        : `${stall.name} added to your interests`,
    });
  };
  ...
  return (
    ...
    style={{paddingBottom: '170px'}} >
      {/* Header */}
      <motion.div
        className="sticky top-0 z-40 bg-gradient-primary text-white p-4 shadow-elevated"
        initial={{ y: -100 }}
        animate={{ y: 0 }}
      ...
      <div className="p-4">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="map" className="space-y-6">
            <motion.div
              className="h-80 rounded-2xl overflow-hidden shadow-card"
            ...
            <div>
              <h3 className="text-lg font-semibold text-foreground mb-4">
                Nearby Stalls ({mockStallData.length})
              </h3>
